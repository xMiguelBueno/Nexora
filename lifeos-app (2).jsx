  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setState(res?.value ? migrateState(JSON.parse(res.value)) : defaultState());
      } catch {
  ...
  async function send() {
    ...
    try {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 1000, system: AI_SYSTEM_PROMPT + "\n\nContexto atual do usuário (JSON):\n" + JSON.stringify(context), messages: history }),
      });
  ...
  return (
    <div className="max-w-lg mx-auto px-6 py-8 space-y-6">
      ...
      <div className="los-elev rounded-xl p-4">
        ...
        <p className="text-xs los-dim leading-relaxed">Tudo é salvo automaticamente no seu armazenamento pessoal deste app. O Mentor IA usa a API da Anthropic diretamente deste ambiente — nenhuma chave fica exposta no código. Em uma versão de produção (Next.js), essa chamada passa por uma rota de servidor própria, com a chave apenas em variável de ambiente.</p>
      </div>
